    ...
    <article class="card">
       <div class="card-body bg-gray-100">
          <form wire:submit.prevent="store">
             <input wire:model="name" type="text" class="form-input w-full" placeholder="Agregar un requerimiento">
             @error('name')
                 <span class="text-xs text-red-400"> {{$message}} </span>
             @enderror
             <div class="flex justify-end mt-2">
                <button class="btn btn-primary" type="submit">Agregar requerimiento</button>
             </div>
          </form>
       </div>
    </article>
